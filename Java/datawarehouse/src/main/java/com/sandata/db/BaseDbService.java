...
public class BaseDbService {
    ...
    public boolean waitDataExistInDb(String sql) {
        ...
        while ( retry > 0 ) {
            if(DbUtilsCon.getDataTable(
                    getTestEnvironment().getOracleUrl(),
                    getTestEnvironment(), sql).size() != 0) {
    ...
    public List<Map<String, Object>> getDataTable(String sql){
        return DbUtilsCon.getDataTable( getTestEnvironment().getOracleUrl(), getTestEnvironment(), sql);
    }
