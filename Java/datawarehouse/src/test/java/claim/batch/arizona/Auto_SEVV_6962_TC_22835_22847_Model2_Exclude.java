...
import static com.interop.common.constants.Constant.ZIP_EXTENSION;
...
public class Auto_SEVV_6962_TC_22835_22847_Model2_Exclude extends ImportBaseTest {

    @Test(groups = {"regression"})
    @QTest(keys = {"TC-22835","TC-22847"})
    public void ExcludeModel2() throws Exception {
        //Prepare Environment
        ...
        List<File> listFileNameUploaded = new ArrayList<>();
        for (File file : importClaimServices.getListFileCSV()) {
            String targetPath = file.getPath() + ZIP_EXTENSION;
            zip(targetPath, file);
            listFileNameUploaded.add(new File(targetPath));
        }
        for (File file : listFileNameUploaded) {
        ...
        boolean isFileResponseDisplayed = isSftpFileExisted(default_upload_folder_dropOff_claim + listFileResponse.get(RESPONSE.getIndex()) + ZIP_EXTENSION, MAX_TIME_ATTEMPT);
        ...
        for (String fileName : listFileResponse) {
            getFile(default_upload_folder_dropOff_claim, FileType.ZIP, fileName, DOWNLOADED_FILES);
            unzip(new File(DOWNLOADED_FILES + fileName + ZIP_EXTENSION), DOWNLOADED_FILES);
