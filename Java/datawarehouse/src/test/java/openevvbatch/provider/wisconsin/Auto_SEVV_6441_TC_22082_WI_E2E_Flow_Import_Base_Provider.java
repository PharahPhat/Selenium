package openevvbatch.provider.wisconsin;
...
public class Auto_SEVV_6441_TC_22082_WI_E2E_Flow_Import_Base_Provider extends ImportBaseTest {
    ImportProviderService importProviderService;


    @Test(groups = {"regression"})
    @QTest(keys = {"TC-23313","TC-23315"})
    public void Import_Base_Provider() throws InterruptedException, SftpException, IOException, SQLException, InvocationTargetException, IllegalAccessException, NoSuchAlgorithmException, KeyManagementException, URISyntaxException, InstantiationException, TimeoutException, NoSuchFieldException {
        importProviderService = new ImportProviderService();

        baseObj.info("Prepare data before starting");
        String providerID = commons.generateUniqueNumber().substring(0, 15);
